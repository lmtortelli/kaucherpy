...
def ackley( x, a=20, b=0.2, c=2*math.pi ):
    ...
    s2 = sum( [cos( c * v ) for v in x])
    
    return -a*exp( -b*( s1 / n )**(1.0/2.0)) - exp( s2 / n ) + a + exp(1)
...
def michalewicz( x ):  # mich.m
    ...
    j = np.arange( 1., n+1 )
    t1 = [sin(v) for v in x]
    t2 = [sin( j[i] * x[i]**2 / math.pi ) for i in range(len(x))]
...
def rastrigin( x ):  # rast.m
    x = np.asarray_chkfinite(x)
    n = len(x)
    return 10*n + sum([ v**2 - 10 * cos( 2 * math.pi * v )  for v in x  ])
...
def schwefel( x ):  # schw.m
    x = np.asarray_chkfinite(x)
    n = len(x)
    return 418.9829*n - sum( [v * sin( km.sqrt( km.abs( v ))) for v in x] )
